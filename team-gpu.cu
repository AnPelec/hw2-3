...
thrust::device_ptr<int> bucket_sizes_ptr;
...
__global__ void compute_forces_gpu(particle_t* parts, int num_parts, int* particles_in_buckets, double size, int grid_side_length, int cnt, int* bucket_sizes)
{
    ...
    {
        int particle_index = particles_in_buckets[offset + i];
        ...
        for (int bx = max(bucket_row-1, 0); bx <= min(bucket_row+1, grid_side_length-1); bx++)
        {
            for (int by = max(bucket_col-1, 0); by <= min(bucket_col+1, grid_side_length-1); by++)
            {
                ...
                int start_index = (neighbor_bucket == 0 ? 0 : bucket_sizes[neighbor_bucket-1]);
                int end_index = bucket_sizes[neighbor_bucket];

                for (int j = start_index; j < end_index; j++)
                {
                    int neighbor_index = particles_in_buckets[offset + j];
...
void init_simulation(particle_t* parts, int num_parts, double size)
{
    ...
    grid_side_length = std::min(int(2*size/(3*cutoff)), int(4*sqrt(num_parts))); // Number of rows/columns in our grid of buckets
    ...
    bucket_sizes_ptr = thrust::device_pointer_cast(bucket_sizes);
    thrust::inclusive_scan(bucket_sizes_ptr,bucket_sizes_ptr + num_buckets, bucket_sizes_ptr);
...
void simulate_one_step(particle_t* parts, int num_parts, double size)
{
    ...
    compute_forces_gpu<<<blks, NUM_THREADS>>>(parts, num_parts, particles_in_buckets, size, grid_side_length, cnt, bucket_sizes);
